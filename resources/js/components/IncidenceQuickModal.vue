      ...
      <Transition name="pop">
        <div class="relative w-full max-w-2xl rounded-xl bg-white shadow-xl dark:bg-slate-900">
          <div class="p-5 border-b border-gray-200 dark:border-slate-800">
            <div class="text-lg font-semibold text-gray-900 dark:text-slate-100">Nueva incidencia</div>
            <div class="mt-1 text-sm text-gray-600 dark:text-slate-300">
              Registra una incidencia y asígnala a un cliente.
            </div>
          </div>
...
const onCustomerInput = () => {
  // if user edits the query after selecting, clear selection.
  form.value.customer_id = null;
  selectedCustomerLabel.value = '';
...
const submit = async () => {
  ...
  try {
    await axios.post('/incidencias', payload);
    toastSuccess('Incidencia creada');
    hide();
    window.dispatchEvent(new CustomEvent('incidencias:created'));
  } catch (e) {
    error.value = firstValidationMessage(e) ?? e?.response?.data?.message ?? 'No se pudo crear la incidencia.';
...
const show = async (detail = {}) => {
  ...
  const customerId = detail?.customer_id ?? detail?.customerId ?? null;
  const customerLabel = detail?.customer_label ?? detail?.customerLabel ?? null;

  if (customerId) {
    form.value.customer_id = Number(customerId);
    if (customerLabel) {
      selectedCustomerLabel.value = String(customerLabel);
      customerQuery.value = String(customerLabel);
    } else {
      await lookupCustomerById(Number(customerId));
    }
  }
};

const onExternalCreate = (e) => {
  show(e?.detail ?? {});
};

onMounted(() => {
  window.addEventListener('incidencias:create', onExternalCreate);
});

onBeforeUnmount(() => {
  window.removeEventListener('incidencias:create', onExternalCreate);
  if (customerTimer) window.clearTimeout(customerTimer);
});
</script>
